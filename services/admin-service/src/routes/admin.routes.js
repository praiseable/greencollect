...
// Services trust X-User-* headers from gateway (no need for authenticate/authorize)
// Gateway already validates JWT and enforces portal guards
const { Portal } = require('../../../../packages/shared/src/constants');

// Admin-only routes - gateway already validates portal and sets X-User-* headers
// Just verify portal from headers
router.use((req, res, next) => {
  if (req.user?.portal !== Portal.ADMIN) {
    return res.status(403).json({
      error: { message: 'Access denied: Admin portal required', code: 'PORTAL_FORBIDDEN' }
    });
  }
  // Check roles from headers
