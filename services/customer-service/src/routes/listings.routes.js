...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    // Apply geo-fencing only when we have OR (dealers get territory; users with zone get visibility-based). No extra filter when no OR.
    if (!isAdmin) {
...
router.get('/favorites', trustHeaders, async (req, res) => {
  try {
    const lang = req.lang || 'en';
...
router.post('/:id/favorite', trustHeaders, async (req, res) => {
  try {
...
router.post('/:id/report', trustHeaders, async (req, res) => {
  try {
...
router.get('/my', trustHeaders, async (req, res) => {
  try {
    const lang = req.lang || 'en';
...
router.post('/', trustHeaders, async (req, res) => {
  try {
    const {
...
router.post('/:id/images', trustHeaders, upload.array('images', 5), async (req, res) => {
  try {
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) return res.status(404).json({ error: { message: 'Listing not found' } });
...
router.put('/:id', trustHeaders, async (req, res) => {
  try {
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) return res.status(404).json({ error: { message: 'Listing not found' } });
    if (listing.sellerId !== req.user.id && !['SUPER_ADMIN', 'ADMIN'].includes(req.user.role)) {
...
router.patch('/:id/deactivate', trustHeaders, async (req, res) => {
  try {
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) return res.status(404).json({ error: { message: 'Listing not found' } });
...
router.patch('/:id/reactivate', trustHeaders, async (req, res) => {
  try {
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) return res.status(404).json({ error: { message: 'Listing not found' } });
...
router.delete('/:id', trustHeaders, async (req, res) => {
  try {
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) return res.status(404).json({ error: { message: 'Listing not found' } });
    if (listing.sellerId !== req.user.id && !['SUPER_ADMIN', 'ADMIN'].includes(req.user.role)) {
...
router.post('/:id/interest', trustHeaders, async (req, res) => {
  try {
    const listing = await prisma.listing.findUnique({ where: { id: req.params.id } });
    if (!listing) return res.status(404).json({ error: { message: 'Listing not found' } });
